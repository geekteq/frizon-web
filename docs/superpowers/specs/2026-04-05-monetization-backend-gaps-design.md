...
**Delivery:** AWS SES v2 via a minimal `SesMailer` service (cURL + SigV4, no SDK/Composer). Sender: `frizon@mobileminds.se` (whitelisted domain). Delivered to `CONTACT_EMAIL` (default: `frizonofsweden@gmail.com`). No DB storage of submissions.
